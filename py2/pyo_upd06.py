...
# --------- Граф с выделением вершин без ребер -------------------
import networkx as nx
import matplotlib.pyplot as plt
from matplotlib.offsetbox import OffsetImage, AnnotationBbox
import PIL
...
icons = {
    "job": "icons/operation.png",
    "team": "icons/group.png",
    "jobnot": "icons/opernot.png",  # Fixed the icon name (removed extra space)
}
...
# Add nodes for jobs
for i in model.I:
    # Check if the job has no edges
    if all(not G.has_edge(f"Team {j}", f"Job {i}") for j in model.J):
...
for node, (x, y) in pos.items():
    if "Team" in node:
        ...
        image = images["jobnot"]  # Use "jobnot" icon for vertices without edges
    im = OffsetImage(image, zoom=0.1)
    ab = AnnotationBbox(im, (x, y), xycoords='data', frameon=False)
    plt.gca().add_artist(ab)
    plt.text(x, y - 0.1, G.nodes[node]['label'], ha='center', fontsize=10)
...
# ---------------Изменение вида графа circular layout ----------------------
import networkx as nx
import matplotlib.pyplot as plt
from matplotlib.offsetbox import OffsetImage, AnnotationBbox
import PIL
...
# Add vertices without edges to the graph layout
for i in model.I:
    if all(not G.has_edge(f"Team {j}", f"Job {i}") for j in model.J):
        G.add_edge(f"Team {j}", f"Job {i}")
...
# ------Изменение цвета вершины (бригады) -----------------
import networkx as nx
import matplotlib.pyplot as plt
from matplotlib.offsetbox import OffsetImage, AnnotationBbox
import PIL
...
# Add nodes for repair teams
for j in model.J:
    # Check if the team has no edges
    if all(not G.has_edge(f"Team {j}", f"Job {i}") for i in model.I):
...
# Add nodes for repair teams
for j in model.J:
    G.add_node(f"Team {j}", color='blue', label=f"Team {j}")

# Add nodes for jobs
for i in model.I:
    G.add_node(f"Job {i}", color='green', label=f"Job {i}")
...
for node, (x, y) in pos.items():
    color = 'blue' if "Team" in node else 'green'
    nx.draw_networkx_nodes(G, pos, nodelist=[node], node_color=color, node_size=300)
    plt.text(x, y - 0.1, G.nodes[node]['label'], ha='center', fontsize=10)
