## Распределение загрузки ремонтных бригад для исполнения графика ППР
...
#### Назначение работ ремонтным бригадам
...
### Выполнение работ вовремя
print("Completion of Jobs on Time:")
for i in model.I:
    print(f"Job {i}: {value(model.z[i])}")

### Бинарная переменная U (способность выполнять работу вовремя)
print("Binary Variable U (Ability to Complete Work on Time):")
for i in model.I:
    for j in model.J:
...
for node, (x, y) in pos.items():
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=[node], node_color=color, node_size=900)
    plt.text(x, y - 0.21, G.nodes[node]['label'], ha='center', fontsize=12)

# Draw edges
nx.draw_networkx_edges(G, pos, edge_color="gray", arrowsize=25)

# Set title
plt.title("Assigned Work for Repair Teams", fontsize=12)
